...
Shrike公式ヘルプページを参照してください。また、筆者の関連する過去作
[＃１（誤字多数・すんません）](ref-forgefpga-trial-report-1)と[＃２](ref-forgefpga-trial-report-2)も参照してください。
SLEG47910Vの一般的な開発手順が書かれています。

# Shrike Lite

Shrikeはホストになるマイコンによって複数種類のバージョンがあり、LiteはRP2040がホストマイコンになっています。
RP2350版はクラウドファンディングが行われており、すぐには入手できない状態です。
RP2040とRP2350はパッケージが異なるのでICだけ載せ替えはできそうにないです。

[@fig:shrike-pinout]にピン配置を示します。

![ピン配置（R0.4・公式ドキュメントより抜粋）](shrike/docs/images/shrike_pinouts.svg){height=158mm #fig:shrike-pinout}
...
SLG47910以下ForgeFPGAシリーズは外部SPIバスからビットストリームをロードする機能が備わっています。バスマスタにもスレーブにもなれます。
...
MCUロードシーケンスの詳細が、旧リビジョンのコンフィグレーションガイドに載っています。新リビジョンにはここまで詳細には書かれていません。
新旧リビジョン間の時期的な差が大きいのでなんとも言えませんが、この部分については新リビジョンは内容が劣化しています。
...
エンドユーザーは気にしなくて良くなったようです（それでも書いておくべきだと思いますが）。なお、フラッシュロード周りはタイミング図もなく、しょぼいままです。
...
[コンフィグレーションガイド](#ref-configuration-guide)にフラッシュメモリのリスト（[@fig:compatible-flash-memory]）が載っていますが、これはおそらく上位品種用です。
...
Listing: main.py (関連部分のみ抜粋) {#lst:main-py-spi1}

MicroPythonのREPL上で[flashboot.store()]{custom-style="PreprocessorTok"}でMCUからフラッシュに転送、
[flashboot.load(mcuboot)]{custom-style="PreprocessorTok"}を打ち込むとフラッシュからのロードが実行されます。
[flashboot.verify()]{custom-style="PreprocessorTok"}でMCUのファイルとフラッシュの内容を比較します。
...
### MCU対フラッシュのアクセスはビットバンギング（SoftSPI）を使う

MCUからFPGAにバイナリを流し込むというShrikeの設計思想にないことを行なっているので、ソフトにも工夫が必要です。
前の章では、MicroPythonスクリプト内でSPIクラスを使いました。SPIクラスはハードウェアの機能のラッパーなので、バス番号を宣言して使います。
SPI1バスを使うと宣言しておいてSPI0のピンを割り当てようとするとエラーになります。下のように、SPI0バス内の役割を入れ替えることもできません。
つまり、MOSIのピンをMISOにすることはできません。

```{.python}
>>> SPI(0,miso=3,mosi=0,sck=2)
...
[`BusPins.py`](data/BusPins.py){.python #lst:buspins-py .listingtable nocaption=true}
...
[`spiflash.py`](data/spiflash.py){.python #lst:spiflash-py .listingtable nocaption=true}
...
[`main.v`](shrike/examples/led_blink/ffpga/src/main.v){.verilog #lst:main-v .listingtable nocaption=true}
...
- [秋月電子/\[112155\]コネクター用ハウジング 7P]{#ref-akizuki-qi-7p}
    - <https://akizukidenshi.com/catalog/g/g112156>
...
本書を最後まで読んでいただきありがとうございます。Shrikeボードやそのクローンボードの設計開発に参考になれば幸いです。
早めに書き始めて早めにさらっとまとめたかったのですが、ズルズルのびたうえ思ったより大作になってしまいました。例によって直前印刷です。

FPGA石がVicharak社から通販購入できるようになったようです。私は試してはいませんが、参考までにリンク集に追記しておきました。秋月も売ってくれー

以下、いつも通りの箇条書きです。

- ソースコードに行番号つけるなどで合わせて5週間くらいフィルタ開発に寄り道してしまったが、後悔はしていない。
- MouserでSLG47910Vを怒りのフルリール**5000個**購入したけど夏コミには間に合いませんでした。ざんねん！
    - ６月初旬時点では単価３８０円くらいでした。Mouserには「あなた専用にしか売らないしキャンセルもお断りです」と念書を書かされました。
      なお総額がとんでもなくなって仕方なくカードの利用枠を増額しました。なにがUXやねんちょっといいオシロ買えるやんけそういうとこやぞ&reg;
