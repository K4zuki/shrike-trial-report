...
FPGAバイナリの生成は本家と同じ方法ですが、ファイルシステムへの転送は
[mpremote]{custom-style="PreprocessorTok"}コマンドを使い、さらにマイコン-FPGA間転送用スクリプト
[McuBoot.py]{custom-style="PreprocessorTok"}を使ってRP2040からFPGAに転送します。
...
[build/bitstream]{custom-style="PreprocessorTok"}フォルダの中にバイナリファイルとテキストファイルが生成されます。
...
ボード上に用意されているテストパッドとSPI1バス（IO8～11）を接続します。テストパッドが小さいので、0.2～0.3mm&phi;のUEW（いわゆるエナメル線）が使いやすいです。
